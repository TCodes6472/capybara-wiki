...
function render(q=""){const found=search(q);function render(q=""){
  const found=search(q);
  grid.innerHTML=found.map(a=>`<a class="card-link" href="animals/${slug(a.name)}.html"><article class="card" tabindex="0" role="link"><div class="icon">${a.icon}</div><h3>${a.name}</h3><div class="scientific">${a.sci}</div><div class="tags"><span class="tag">${a.group}</span><span class="tag">${a.diet}</span></div><p class="description">${a.desc}</p><div class="facts"><span>${a.habitat}</span>${a.traits.slice(0,2).map(t=>"<span>• "+t+"</span>").join("")}</div></article></a>`).join("");
  count.textContent=found.length+" animals";
  empty.hidden=found.length>0;
  status.textContent=q?("Results for “"+q+"”"):("Showing all "+animals.length+" profiles");
}
count.textContent=found.length+" animals";empty.hidden=found.length>0;status.textContent=q?("Results for \u{e2}\u{80}\u{9c}"+q+"\u{e2}\u{80}\u{9d}"):("Showing all "+animals.length+" profiles")}
...
(function(){
  ...
  render=function(q){
    ...
    grid.innerHTML=found.map(function(a){
      return '<a class="card-link" href="animals/'+slug(a.name)+'.html"><article class="card" tabindex="0" role="link" aria-label="Explore '+esc2(a.name)+'"><div class="icon">'+iconFor(a)+'</div><h3>'+esc2(a.name)+'</h3><div class="scientific">'+esc2(a.sci)+'</div><div class="tags"><span class="tag">'+esc2(a.group)+'</span><span class="tag">'+esc2(a.diet)+'</span></div><p class="description">'+esc2(a.desc)+'</p><div class="facts"><span>Habitat: '+esc2(a.habitat)+'</span>'+a.traits.slice(0,2).map(function(t){return '<span>• '+esc2(t)+'</span>';}).join("")+'</div></article></a>';
    }).join("");
    count.textContent=found.length+" animals";
