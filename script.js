...
function render(q=""){const found=search(q);grid.innerHTML=found.map(a=>`<article class="card"><div class="icon">${a.icon}</div><h3>${a.name}</h3><div class="scientific">${a.sci}</div><div class="tags"><span class="tag">${a.group}</span><span class="tag">${a.diet}</span></div><p class="description">${a.desc}</p><div class="facts"><span>\u{f0}\u{9f}\u{93}\u{8d} ${a.habitat}</span>${a.traits.slice(0,2).map(t=>`<span>\u{e2}\u{80}\u{a2} ${t}</span>`).join("")}</div></article>`).join("");count.textContent=found.length+" animals";empty.hidden=found.length>0;status.textContent=q?("Results for \u{e2}\u{80}\u{9c}"+q+"\u{e2}\u{80}\u{9d}"):("Showing all "+animals.length+" profiles")}
